...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error === AUTH_UNAVAILABLE_ERROR) {
        setError("Login is temporarily unavailable. Please check the local database and try again.");
      ...
      const message =
        err instanceof Error && err.message === AUTH_UNAVAILABLE_ERROR
          ? "Login is temporarily unavailable. Please check the local database and try again."
  ...
  return (
    <div className="min-h-screen bg-navy-900 mesh-blue flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        {/* Logo lockup */}
        <div className="flex flex-col items-center gap-3 mb-8">
          <div className="flex items-center gap-3">
            <svg viewBox="0 0 520 521" className="w-10 h-10 shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="font-display font-black tracking-[0.05em] text-2xl text-white uppercase">
              EpicenTra
            </span>
          </div>
          <p className="text-xs text-brand-coral font-semibold tracking-wider uppercase">
        ...
        <div className="bg-navy-800 rounded-2xl border border-navy-700 p-8 shadow-2xl">
          <h1 className="font-display font-bold text-xl text-white mb-1 uppercase tracking-wide">Welcome back</h1>
